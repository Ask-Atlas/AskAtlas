...
// files + the chunk table from this PR -- not the full migration
// history), and exercises the sqlc-generated queries against real
// Postgres.
//
// Skipped by default: the build tag `integration` keeps it out of
// `go test ./...` so contributors without Docker can still run unit
// tests. Run locally with `make test-integration` (or
